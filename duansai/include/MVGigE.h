 ...
 *  \brief 设置查找表
 *  \param [in]	HANDLE hCam	相机句柄
 *  \param [in]	unsigned long * pLUT	查找表数组，unsigned long pLUT[1024];
 *  \param [in]	int nCnt	查找表数组单元个数，必须是1024
 *  \retval  	MVST_SUCCESS			: 成功
 */
 ...
 *  \brief 获取当前是否使用查找表状态
 *  \param [in]	HANDLE hCam
 *  \param [out]	BOOL * bEnable	当前是否使用查找表状态
 *  \retval  	MVST_SUCCESS			: 成功
 */
MVGIGE_API MVSTATUS_CODES __stdcall MVGetEnableLUT( HANDLE hCam,BOOL *bEnable );
	 ...
	 *  \brief 采集一帧图像。
	 *  \param [in]	HANDLE hCam	相机句柄
	 *  \param [out]	HANDLE hImage	图像句柄。保存采集到的图像。
	 *  \param [in]	unsigned long nWaitMs	等待多长时间，单位ms
	 *  \retval  	MVST_SUCCESS			: 成功
	 */
	 ...
	 *  \param [in]	HANDLE hCam
	 *  \param [out]	unsigned long * pDroppedFrames	扔掉的帧数
	 *  \retval  	MVST_SUCCESS			: 成功
	 */
